...
const PROJECT_DIR = resolve(homedir(), 'sela');
...
const BOT_MEMORY_PATH = join(homedir(), 'sela', 'MEMORY.md');
const MCP_CONFIG = join(homedir(), 'sela', 'mcp-config.json');
const CLI_TIMEOUT = config.cliTimeout;

// --- Cost tracking ---

const COSTS_FILE = join(homedir(), 'sela', 'data', 'costs.jsonl');
mkdirSync(join(homedir(), 'sela', 'data'), { recursive: true });
...
function spawnClaude(args, stdinContent, onTextDelta, onToolUse) {
  return new Promise((resolve, reject) => {
    ...
    const MAX_TIMEOUT = CLI_TIMEOUT;                     // 900s default — absolute safety net
...
async function buildContext(userText, { tier = 2, history = [], jid, isGroup, groupContext } = {}) {
  ...
  if (matchedSkillNames.length > 0) {
    const SKILL_CONTEXT_LIMIT = 2000; // 2KB per skill — send summary, not full content
    const skillParts = [];
  ...
  const memResult = await memorySearch(userText, {
    tier, maxTokens: 3000, profile,
    includeIntentions: needsIntentions,
    includeTodayNotes: tier >= 3,
...
export async function chatOneShot(prompt, onChunk, modelOverride = null, { cronId, cronState, onToolUse } = {}) {
  const pipelineStart = Date.now();
