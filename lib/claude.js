...
setInterval(() => {
  ...
  // Rebuild system prompt if SOUL/MEMORY files changed
...
// Dynamic system prompt: base + SOUL.md + MEMORY.md (cached by API, not re-sent as user content)
...
async function buildContext(userText, { tier = 2, history = [], jid, isGroup, groupContext } = {}) {
  ...
  // Goals: inject for tier >= 2 (skip for status-only queries and follow-ups)
  ...
  const cronRelevant = !isFollowup && (profile === 'cron' || tier >= 3 || /cron|schedul|timer|automat|recurring|תזמון|קרונ/i.test(userText));
