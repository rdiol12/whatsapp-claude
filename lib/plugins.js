...
const PLUGINS_DIR = join(homedir(), 'sela', 'plugins');
 ...
 * Returns { text, extraContext, handled? } after all plugins have processed.
 * If a plugin returns { handled: true }, the pipeline stops and the message
 * should NOT be sent to Claude (the plugin already responded).
 */
export async function runPreChatPipeline(text, history, botApi) {
  let current = { text, extraContext: '' };
  for (const p of plugins) {
    if (p.enabled && p.hooks.preChat) {
      const snapshot = current.text; // preserve text before plugin runs
      try {
        const result = await p.hooks.preChat(current.text, history, botApi);
        if (result && typeof result === 'object') {
          if (result.handled) {
            log.info({ plugin: p.name }, 'Plugin intercepted message');
        ...
        current.text = snapshot; // restore text on plugin error
        log.error({ plugin: p.name, hook: 'preChat', err: err.message }, 'Plugin preChat error (text restored)');
      }
    }
  }
  return current;
