...
import { writeFileSync, readFileSync, readdirSync, statSync, mkdirSync, chmodSync } from 'fs';
...
import { registerBroadcast } from './ws-events.js';
import { getAgentLoopStatus, getAgentLoopDetail } from './agent-loop.js';
...
const IPC_TOKEN = randomBytes(24).toString('hex');
let server = null;
...
async function handleRequest(req, res) {
  ...
  try {
    ...
    if (path === '/tests/run' && method === 'POST') {
      ...
      const testDir = join(homedir(), 'sela', 'test');
      return new Promise((resolve) => {
        execFile('node', [join(testDir, 'run-all.js')], { timeout: 30000 }, (err, stdout, stderr) => {
    ...
    // --- Agent Learning ---
    if (path === '/agent-learning' && method === 'GET') {
      return jsonResponse(res, getEffectivenessStats());
    }
...
function gatherLiveState() {
  ...
  return {
    ...
    costs: costOverview,
    agentLoop: getAgentLoopStatus(),
    agentLearning: getEffectivenessStats(),
  };
}
...
export function startIpcServer({ queueStats } = {}) {
  ...
  server.listen(0, '127.0.0.1', () => {
    ...
    writeFileSync(PORT_FILE, JSON.stringify({ port, token: IPC_TOKEN }));
