...
function finishCurrentMessage() {
  ...
  // Reset accumulators for next message
  fullText = '';
  resultEvent = null;
  currentMessage = null;
...
function drainQueue() {
  ...
  msg.timeout = setTimeout(() => {
    ...
    // Kill + respawn only if 2 consecutive timeouts (process might be stuck)
    ...
    // If 3+ consecutive, MCP might be stuck — full respawn
  ...
  if (!ok) {
    // Backpressure: wait for drain before considering process ready again
