...
_Example: "what are my goals?" = /goals_`;
  ...
  async recap(sock, sender, params, msg, botApi, sentMessageIds) {
    await reply(sock, sender, '_Generating recap..._', sentMessageIds);
    try {
      const { text, generatedAt } = await generateRecap();
      const time = new Date(generatedAt).toLocaleTimeString('en-IL', {
        timeZone: 'Asia/Jerusalem', hour: '2-digit', minute: '2-digit',
      });
      await reply(sock, sender, `*Daily Recap* (${time})\n\n${text}`, sentMessageIds);
    } catch (err) {
      await reply(sock, sender, `Recap failed: ${err.message}`, sentMessageIds);
    }
    return true;
  },
