...
const INTENTS = {
  ...
  RECAP: {
    action: 'recap',
    patterns: [
      /^(?:recap|ריקאפ|סיכום\s*יומי)[\s?!.]*$/i,
      /(?:daily|day'?s?|end\s*of\s*day)\s*recap/i,
      /(?:summarize|sum\s*up)\s*(?:the\s*)?(?:day|today)/i,
      /(?:תסכם|סכם)\s*(?:לי\s*)?(?:את\s*)?(?:היום|העבודה)/,
      /מה\s*(?:עשינו|עברנו|הספקנו)\s*היום/,
    ],
  },
