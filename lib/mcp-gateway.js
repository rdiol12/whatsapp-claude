...
let closing = false;
let offlineUntil = 0; // circuit breaker: fast-fail until this timestamp
...
async function ensureConnected() {
  ...
  // Circuit breaker: fast-fail if we recently failed (avoids blocking every call)
  if (Date.now() < offlineUntil) {
    throw new Error(`vestige-mcp circuit breaker open (retry in ${Math.ceil((offlineUntil - Date.now()) / 1000)}s)`);
  }
...
async function reconnectWithBackoff() {
  ...
  try {
    ...
    offlineUntil = 0; // circuit breaker: reset on success
  } catch (err) {
    consecutiveFailures++;
    // Circuit breaker: block reconnect attempts for 60s after failure
