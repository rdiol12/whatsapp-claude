...
import { writeFileAtomic, cleanupOrphanedTempFiles } from './resilience.js';

const log = createLogger('state');
const STATE_DIR = join(config.dataDir, 'state');

// Ensure state dir exists and clean up any orphaned temp files from previous crashes
mkdirSync(STATE_DIR, { recursive: true });
cleanupOrphanedTempFiles(STATE_DIR);

// --- In-memory cache (write-through) ---
// Safe from race conditions: all operations are synchronous (readFileSync/writeFileSync),
