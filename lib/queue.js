...
export function createQueue({ maxConcurrent = 2, maxQueuePerUser = 5 } = {}) {
  ...
  /**
   * Acquire a concurrency slot directly (for crons and other non-user tasks).
   * Must call releaseSlot() when done.
   */
  async function acquireSlot() {
    await acquire();
  }

  /** Release a slot acquired via acquireSlot(). */
  function releaseSlot() {
    release();
  }

  return { enqueue, stats, drain, acquireSlot, releaseSlot };
}
