  ...
  try {
    ...
    if (triggered && triggered.length > 50) {
      ...
      try {
        ...
        if (parsed.triggered && parsed.triggered.length === 0 &&
            parsed.pending && parsed.pending.length === 0) {
          // Nothing actually triggered — skip
        } else {
          throw new Error('has content'); // fall through to send
        }
      } catch {
        // Not JSON or has actual content — send it
