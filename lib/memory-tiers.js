...
// --- Tier thresholds ---
const T1_THRESHOLD = 0.7;
const T2_THRESHOLD = 0.4;
...
function calcWeight(entry) {
  ...
  // Frequency bonus: 1 + min(mentionCount * 0.1, 0.5) — caps at 1.5x
  const freq = 1 + Math.min((entry.mentionCount || 0) * 0.1, 0.5);

  // Recency factor: max(0.3, 1.0 - weeksSince * 0.05) — 5%/week decay, floor 0.3
  const weeksSince = (Date.now() - (entry.lastAccessed || entry.firstSeen || Date.now())) / (7 * 24 * 3600_000);
  const recency = Math.max(0.3, 1.0 - weeksSince * 0.05);
...
export function rankResults(rawText) {
  ...
  for (const line of lines) {
    ...
    const item = { text: line.trim(), weight, tier };
    if (tier === 1) buckets.t1.push(item);
    else if (tier === 2) buckets.t2.push(item);
