...
import { getEffectivenessStats } from './agent-learning.js';

const log = createLogger('self-review');
const SOUL_PATH = join(homedir(), 'sela', 'SOUL.md');
...
function gatherReviewData() {
  ...
  let agentLoopStats = null;
  try {
    agentLoopStats = getEffectivenessStats();
  } catch {}

  return { cronData, proposalData, sentimentSummary, previousNotes, agentLoopStats };
}
...
function buildReviewPrompt(currentSection, data) {
  const { cronData, proposalData, sentimentSummary, previousNotes, agentLoopStats } = data;
  ...
  return `You are reviewing your own performance as a personal AI agent for Ron.
...
${agentLoopStats && agentLoopStats.totalCycles > 0 ? `## Agent Loop Stats (last ${agentLoopStats.totalCycles} cycles):
- Actions/signal ratio: ${agentLoopStats.signalToActionRatio}
- Avg cost per cycle: $${agentLoopStats.avgCostPerCycle}
- Total cost: $${agentLoopStats.totalCost}
- Message engagement rate: ${agentLoopStats.engagementRate !== null ? agentLoopStats.engagementRate + '%' : 'no data'}
- Top signals: ${agentLoopStats.topSignalTypes.map(t => `${t.type}(${t.count})`).join(', ') || 'none'}` : ''}
