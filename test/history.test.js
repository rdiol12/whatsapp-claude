...
const JID = 'test-history-' + Date.now() + '@s.whatsapp.net';
...
const JID2 = 'test-ack-' + Date.now() + '@s.whatsapp.net';
...
test('acks in older messages are filtered', () => {
  const jid = 'test-ack-filter-' + Date.now() + '@s.whatsapp.net';
...
test('ack confirming a decision is kept', () => {
  const jid = 'test-ack-decision-' + Date.now() + '@s.whatsapp.net';
...
test('first message in result is always user role', () => {
  const jid = 'test-role-' + Date.now() + '@s.whatsapp.net';
  addMessage(jid, 'user', 'hello');
...
test('respects maxTokens budget', () => {
  const jid = 'test-budget-' + Date.now() + '@s.whatsapp.net';
...
test('trackTopic extracts meaningful words', () => {
  const jid = 'test-topic-' + Date.now() + '@s.whatsapp.net';
...
test('trackTopic ignores stopwords', () => {
  const jid = 'test-topic-stop-' + Date.now() + '@s.whatsapp.net';
...
test('clear removes all messages', () => {
  const jid = 'test-clear-' + Date.now() + '@s.whatsapp.net';
  addMessage(jid, 'user', 'hello');
  addMessage(jid, 'assistant', 'hi');
  clear(jid);
  expect(getMessages(jid).length).toBe(0);
